...
This enables predicting symbol X using features from all symbols (including X).
"""
...
class FeatureBuilder:
    ...
    def symbols(self) -> List[str]:
        ...
        if self._symbols is None:
            csv_files = sorted(self.weekly_data_dir.glob("*.csv"))
            self._symbols = [f.stem for f in csv_files if not f.name.startswith("_")]
    ...
    def load_weekly_data(self) -> Dict[str, pd.DataFrame]:
        ...
        """
        logger.info(f"Loading weekly data for {len(self.symbols)} symbols...")
    ...
    def build_feature_matrix(
        ...
        - Columns = {symbol}_{feature} for all symbol/feature combinations

        Args:
            align_to_common_range: If True, only include weeks present in all symbols

        Returns:
            DataFrame with shape (n_weeks, n_symbols * n_features)
        """
        if not self._weekly_data:
            self.load_weekly_data()
        ...
        for symbol in sorted(self._weekly_data.keys()):
        ...
        n_symbols = len(self._weekly_data)
        n_features = len(self.features)
        logger.info(
            f"Feature matrix shape: {feature_matrix.shape} "
            f"({len(all_weeks)} weeks × {n_symbols} symbols × {n_features} features)"
        )
    ...
    def build_target_matrix(
        ...
        Creates a matrix of future returns for each symbol, shifted by prediction_horizon.
        ...
        Returns:
            DataFrame with shape (n_weeks, n_symbols)
        """
    ...
    def save(self, output_dir: Path) -> None:
        ...
        # Save metadata
        metadata = {
            "symbols": list(self._weekly_data.keys()),
    ...
    def load(cls, features_dir: Path) -> "FeatureBuilder":
        ...
        builder = cls(weekly_data_dir=features_dir)  # Dummy path
