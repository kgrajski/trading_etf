#!/usr/bin/env python3
"""
19d-run-bear-boost-grid.py

Systematic grid search with BEAR MARKET capital boost.

Key difference from 19c:
- Detects market regime using SPY 50-day moving average
- Bear market (SPY < 50-day MA): Increase position size by 10%
- Bull market: Standard position sizing

Rationale: Mean-reversion appears stronger in bear markets where
oversold conditions create better bounce opportunities.
...
Outputs:
- experiments/exp019_grid_bear_boost/grid_results.csv
- experiments/exp019_grid_bear_boost/grid_heatmaps.html
- experiments/exp019_grid_bear_boost/best_configs.json
- experiments/exp019_grid_bear_boost/regime_analysis.html
"""
...
EXPERIMENT_NAME = "exp019_grid_bear_boost"  # Bear market capital boost
...
BEAR_MARKET_MULTIPLIER = 1.10  # 10% larger positions in BEAR markets (flipped from 19c)
...
def run_backtest_with_data(
    ...
    for week_idx, (week_start, week_end) in enumerate(trading_weeks):
        ...
        if is_bull_week:
            bull_weeks += 1
            position_multiplier = 1.0  # Standard sizing in bull markets
        else:
            bear_weeks += 1
            position_multiplier = BEAR_MARKET_MULTIPLIER  # 1.10x in BEAR markets
...
@workflow_script("19d-run-bear-boost-grid")
def main() -> None:
    ...
    try:
        ...
        if len(results_df) > 0:
            ...
            print(f"  Position Multiplier in Bear: {BEAR_MARKET_MULTIPLIER}x")
            print("=" * 70)
        else:
