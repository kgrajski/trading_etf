#!/usr/bin/env python3
"""
19b-run-backtest-grid.py

Systematic grid search over mean-reversion backtest hyperparameters using Ray.
...
Outputs:
- experiments/exp019_grid/grid_results.csv
- experiments/exp019_grid/grid_summary.html (heatmaps)
- experiments/exp019_grid/best_configs.json
"""
...
EXPERIMENT_NAME = "exp019_grid_v2"  # v2: Friday exit at open price
...
def load_shared_data():
    ...
    Returns:
        Dict with 'daily' and 'weekly' DataFrames by symbol
    """
...
def run_backtest_with_data(
    ...
    """
    Run a single backtest with pre-loaded data.
    
    This is the core backtest logic extracted to work with shared data.
    """
    # Get trading weeks
    all_weeks = set()
    ...
    for week_idx, (week_start, week_end) in enumerate(trading_weeks):
        ...
        for order in pending_orders:
            ...
            if row["low"] <= limit_price:
                ...
                target_size = INITIAL_CAPITAL / MAX_ACTIVE_TRADES
...
@workflow_script("19b-run-backtest-grid")
def main() -> None:
    """Run grid search using Ray."""
